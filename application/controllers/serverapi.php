...
class ServerApi extends CI_Controller {
    ...
    function registerStaff(){
        ...
        $this->db->or_where('staff_hp',$hp);
        $q = $this->db->get('tb_staff');
    if($q -> num_rows() > 0){
    $data['message'] = 'Email atau Hp sudah terdaftar, silahkan Sign In';
    $data['status'] = 404;
    } else{
        $simpan['staff_name'] = $nama;
        $simpan['staff_email'] = $email;
        $simpan['staff_password'] = md5($password);
        $simpan['staff_hp'] = $hp;
        $simpan['staff_alamat'] = $alamat;
        $q = $this->db->insert('tb_staff', $simpan);
    if($q){
